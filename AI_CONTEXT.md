...
Le bouton « Télécharger le contenu » telecharge une archive `<id>.zip`. L'image source est convertie en JPEG dans le navigateur et l'archive contient exactement `<id>/image.jpg` et `<id>/question.json`, dont le JSON reference `"image": "image.jpg"`. Le ZIP peut etre decompresse directement dans `content/`. Aucun brouillon n'est enregistre dans `localStorage`, une base de donnees ou un serveur ; un rafraichissement avant export peut donc effacer le brouillon, volontairement.
